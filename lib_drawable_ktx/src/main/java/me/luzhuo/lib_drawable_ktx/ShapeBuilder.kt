...
import me.luzhuo.lib_core_ktx.parseColor
import me.luzhuo.lib_drawable_ktx.DrawableBuilder.Companion.dp2px
...
class ShapeBuilder : DrawableBuilder {
     ...
     */
    fun corner(leftTop: Float, rightTop: Float, leftBottom: Float, rightBottom: Float, isDp: Boolean = true): ShapeBuilder {
    ...
    fun padding(left: Float, top: Float, right: Float, bottom: Float, isDp: Boolean = true): ShapeBuilder {
        mPadding = Rect().apply {
            if (isDp) {
                set(dp2px(left).roundToInt(), dp2px(top).roundToInt(), dp2px(right).roundToInt(), dp2px(bottom).roundToInt())
            } else {
    ...
    fun padding(padding: Float, isDp: Boolean = true): ShapeBuilder {
        return padding(padding, padding, padding, padding, isDp)
    }
    ...
    fun solidRes(@ColorRes colorId: Int): ShapeBuilder {
        return solid(getColor(colorId))
    }
    ...
    fun solid(colorString: String): ShapeBuilder {
        return solid(colorString.parseColor() ?: 0)
    }
     ...
     * @param colorString 颜色的string值
     */
    fun solidRes(colorString: String?, @ColorRes defaultColorId: Int): ShapeBuilder {
        return solid(colorString.parseColor() ?: getColor(defaultColorId))
    }

    fun solid(colorString: String?, @ColorInt defaultColor: Int): ShapeBuilder {
        return solid(colorString.parseColor() ?: defaultColor)
    }
     ...
     * @param shape 形状
     */
    fun shape(shape: ShapeType): ShapeBuilder {
        mShape = when (shape) {
     ...
     */
    @JvmOverloads
    fun strokeRes(@ColorRes colorId: Int, dpWidth: Float, dpDashGap: Float = 0f, dpDashWidth: Float = 0f, isDp: Boolean = true): ShapeBuilder {
        return stroke(getColor(colorId), dpWidth, dpDashGap, dpDashWidth, isDp)
    }
     ...
     */
    @JvmOverloads
    fun stroke(colorString: String, width: Float, dashGap: Float = 0f, dashWidth: Float = 0f, isDp: Boolean = true): ShapeBuilder {
        return stroke(colorString.parseColor() ?: 0, width, dashGap, dashWidth, isDp)
    }
     ...
     */
    @JvmOverloads
    fun stroke(colorInt: Int, width: Float, dashGap: Float = 0f, dashWidth: Float = 0f, isDp: Boolean = true): ShapeBuilder {
    ...
    @JvmOverloads
    fun gradient(angle: Int, startColor: String, endColor: String, centerColor: String? = null): ShapeBuilder {
        return gradient(angle, startColor = startColor.parseColor() ?: 0, endColor = endColor.parseColor() ?: 0, centerColor = if (centerColor == null) null else centerColor.parseColor() ?: 0)
    }
     ...
     */
    @JvmOverloads
    fun gradientRes(angle: Int, @ColorRes startColor: Int, @ColorRes endColor: Int, @ColorRes centerColor: Int? = null, ): ShapeBuilder {
        return gradient(angle, startColor = getColor(startColor), endColor = getColor(endColor), centerColor = if (centerColor == null) null else getColor(centerColor))
    }
     ...
     */
    @JvmOverloads
    fun gradient(angle: Int, startColor: Int, endColor: Int, centerColor: Int? = null, ): ShapeBuilder {
     ...
     * 设置渐变色
     *
     * @param colors    不限数量的渐变色
     * @param angle     渐变角度，必须为45的倍数
     */
    fun gradient(angle: Int, colors: IntArray): ShapeBuilder {
        require(angle % 45 == 0) { "'angle' attribute to be a multiple of 45" }
        mGradientColor = colors
        mAngle = angle
        return this
    ...
    companion object {
        private const val CLASS_GRADIENT_STATE = "android.graphics.drawable.GradientDrawable\$GradientState"
